...
[cmdletbinding(DefaultParameterSetName = 'Capable')]
    ...
    Write-Verbose ('Writing logs to: {0}' -f $LogFile) -Verbose
...
function IsDomainController
{
    ...
    foreach ($ObjItem in $CompConfig) 
    {
        ...
        Switch ($Role) 
        {
            0 { Write-Log "Standalone Workstation" }
            1 { Write-Log "Member Workstation" }
            2 { Write-Log "Standalone Server" }
            3 { Write-Log "Member Server" }
            4 
            {
            ...
            default { Write-Log "Unknown Domain Role" }
        }
    }
    return $_isDC
}
...
{
    Write-Verbose 'Tool to check if your device is capable to run Device Guard and Credential Guard.'
...
{
Write-Warning 'Help has been moved to a command base help'
